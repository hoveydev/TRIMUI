...
# Update MinUI's recent.txt so the actual ROM appears in Recently Played
# rather than the "0) Random Game (RND)" selector entry that MinUI wrote
# before launching this script.
update_minui_recent() {
    local ROM_PATH="$1"
    # Build the relative path MinUI uses (strip /mnt/SDCARD prefix)
    local REL_PATH="${ROM_PATH#/mnt/SDCARD}"
    # Display name: basename without the last extension
    local BASENAME
    BASENAME=$(basename "$ROM_PATH")
    local DISPLAY_NAME="${BASENAME%.*}"

    local TMP_FILE="${RECENT_FILE}.tmp"
    # Write the new entry first
    printf '%s\t%s\n' "$REL_PATH" "$DISPLAY_NAME" > "$TMP_FILE"
    # Append existing entries, skipping: RND selector, and any duplicate of this ROM
    if [ -f "$RECENT_FILE" ]; then
        grep -v "Random Game (RND)" "$RECENT_FILE" \
            | grep -v "^${REL_PATH}	" \
            | head -n $((RECENT_MAX - 1)) >> "$TMP_FILE"
    fi
